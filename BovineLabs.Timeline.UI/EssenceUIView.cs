...
namespace BovineLabs.Timeline.UI
{
    ...
    public class EssenceUIView : View<EssenceUIViewModel>, IDisposable
    {
        ...
        {
            this.intrinsics = Build(this.MakeIntrinsic, this.BindIntrinsic, this.ViewModel.Intrinsics);
            this.stats = Build(this.MakeStat, this.BindStat, this.ViewModel.Stats);
            this.events = Build(this.MakeEvent, this.BindEvent, this.ViewModel.Events);

            this.Add(Section("Intrinsics", this.intrinsics));
            this.Add(Section("Stats", this.stats));
            this.Add(Section("Events", this.events));
        ...
        private VisualElement MakeIntrinsic()
        {
            ...
            var fill = new VisualElement { name = "fill", style = { height = Length.Percent(100), width = Length.Percent(0), backgroundColor = FillColor } };
            track.Add(fill);
        ...
        {
            var row = this.ViewModel.Value.Intrinsics[index];
        ...
        {
            var row = this.ViewModel.Value.Stats[index];
            element.Q<Text>("label").text = row.Label;
        ...
        {
            var row = this.ViewModel.Value.Events[index];
        ...
        private void OnPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            switch (e.PropertyName)
            {
                case nameof(EssenceUIViewModel.Intrinsics):
                    this.intrinsics.itemsSource = this.ViewModel.Intrinsics;
                    this.intrinsics.Refresh();
                    break;
                case nameof(EssenceUIViewModel.Stats):
                    this.stats.itemsSource = this.ViewModel.Stats;
                    this.stats.Refresh();
                    break;
                case nameof(EssenceUIViewModel.Events):
                    this.events.itemsSource = this.ViewModel.Events;
                    this.events.Refresh();
                    break;
            }
        }
